#!/usr/bin/env python3
"""
MVP Docling Pipeline - Progressive Enhancement with Docling
==========================================================
A pipeline that uses Docling for document conversion with fallback performance optimization.

Steps:
1. CONVERSION: Convert documents to markdown using Docling (mvp-docling-core)
...
Usage:
  python mvp-docling-pipeline-progressive.py <input> --step conversion
  python mvp-docling-pipeline-progressive.py <input> --step classification
  python mvp-docling-pipeline-progressive.py <input> --step enrichment
  python mvp-docling-pipeline-progressive.py <input> --step extraction
  python mvp-docling-pipeline-progressive.py <input> --full  # Run all steps
"""
...
class MVPDoclingPipeline:
    """Progressive enhancement pipeline using Docling for conversion."""
    
    def __init__(self, config_path: str = None):
        if config_path is None:
            # Default to .config directory within core_docling
            current_dir = os.path.dirname(os.path.abspath(__file__))
            config_path = os.path.join(current_dir, ".config", "mvp-docling-pipeline-config.yaml")
...
🚀 CONVERSION ENGINE: Docling with PyMuPDF fallback
        """)
        ...
        if self.config and 'pipeline' in self.config and 'performance_targets' in self.config['pipeline']:
            ...
            print(f"""
⚡ PERFORMANCE TARGETS:
   Conversion:     {targets.get('conversion', 'N/A')} pages/sec (Docling + fallback)
   ...
   Extraction:     {targets.get('extraction', 'N/A')} pages/sec
        ...
        print("🚀 Starting Docling pipeline execution...\n")
    ...
    def run_step(self, step_name: str, command: str, description: str) -> Tuple[bool, float, int]:
        ...
        if step_name == "conversion":
            # Parse mvp-docling-core output for real metrics
            total_files = 0
            for line in result.stdout.split('\n'):
                line = line.strip()
                
                # Parse file counts from progress lines
                if 'Progress:' in line and '/' in line:
                    try:
                        # Extract from lines like "📊 Progress: 15/20 files (1.5 files/sec)"
                        progress_part = line.split('Progress:')[1].split('(')[0].strip()
                        total_files = int(progress_part.split('/')[1].split()[0])
                    except:
                        pass
                
                elif 'Avg Speed:' in line and 'pages/sec' in line:
                    try:
                        # Extract from lines like "   Avg Speed:      729.1 pages/sec"
                        parts = line.split('Avg Speed:')[1].split('pages/sec')[0].strip()
        ...
        elif step_name in ["classification", "enrichment", "extraction"]:
            # Parse output for these steps (same as original implementation)
            for line in result.stdout.split('\n'):
                if 'files processed' in line.lower() or step_name in line.lower():
                    try:
    ...
    def step_conversion(self, input_path: str, output_path: str) -> bool:
        """Step 1: Convert documents to markdown using mvp-docling-core."""
        # Use the Docling-based config
        import os
        current_dir = os.path.dirname(os.path.abspath(__file__))
        config_path = os.path.join(current_dir, ".config", "mvp-docling-config.yaml")
        core_path = os.path.join(current_dir, "mvp-docling-core.py")
        
        # Build command with Docling-specific options
        command_parts = [
            f"python {core_path}",
            input_path,
        ...
        if self.config.get('docling', {}).get('use_gpu', False):
            command_parts.append("--gpu")
        
        # Add fallback threshold if configured
        fallback_threshold = self.config.get('docling', {}).get('fallback_threshold', 5.0)
        command_parts.append(f"--fallback-threshold {fallback_threshold}")
        
        command = " ".join(command_parts)
        success, elapsed, files = self.run_step(
            "conversion",
            command,
            "Converting documents using Docling (with PyMuPDF fallback)"
        )
        return success
    ...
    def print_performance_summary(self, total_time: float):
        ...
        print(f"""
╔═══════════════════════════════════════════════════════════════╗
║               DOCLING PIPELINE PERFORMANCE SUMMARY           ║
╚═══════════════════════════════════════════════════════════════╝
...
────────────────────────────────────────────────────────────────""")
        ...
        targets = {
            'conversion': 500,      # Lower target due to Docling's higher quality
            'classification': 2000,
            'enrichment': 1500,
            'extraction': 300
        }
...
✨ Docling Pipeline complete! Check output directories for results.
🎯 Note: Conversion target is lower due to Docling's higher quality output
        """)


def main():
    parser = argparse.ArgumentParser(
        description="MVP Docling Pipeline - Progressive Enhancement with Docling",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Progressive Enhancement Pipeline with Docling:
  Step 1: Conversion     - Documents → Markdown using Docling (with fallback)
  ...
  Step 4: Extraction     - Generate semantic JSON files in same directory

Examples:
  # Full pipeline
  python mvp-docling-pipeline-progressive.py input/ --output output/
  
  # Individual steps (working with markdown directory)
  python mvp-docling-pipeline-progressive.py input/ --output output/ --step conversion
  ...
  python mvp-docling-pipeline-progressive.py output/ --output output/ --step extraction
        """
    )
    
    parser.add_argument("input", nargs='?', help="Input directory (documents for conversion, markdown for other steps)")
    parser.add_argument("--output", default="output", help="Output base directory")
    parser.add_argument("--config", default="mvp-docling-pipeline-config.yaml", help="Configuration file")
    ...
    # Initialize pipeline
    pipeline = MVPDoclingPipeline(args.config)
    ...
    if args.full or (not args.step):
        ...
        # Convert documents to unified output directory
