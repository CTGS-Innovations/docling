#!/usr/bin/env python3
"""
Regex Performance Comparison Test
==================================
...
class HyperscanExtractor:
    ...
    def __init__(self):
        ...
        # Pattern definitions for Hyperscan (bytes)
        patterns = [
            (rb'\$[\d,]+(?:\.?\d{0,2})?', 0),  # MONEY
            (rb'\b\d{1,2}[/-]\d{1,2}[/-]\d{2,4}', 1),  # DATE
            (rb'[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}', 2),  # EMAIL
            (rb'\(?[0-9]{3}\)?[-.\s]?[0-9]{3}[-.\s]?[0-9]{4}', 3),  # PHONE
            (rb'\d{1,3}(?:\.\d{1,2})?%', 4),  # PERCENT
            (rb'\d{1,2}:\d{2}(?::\d{2})?', 5),  # TIME
            (rb'https?://[^\s<>"{}|\\^`\[\]]+', 6),  # URL
            (rb'v?\d+\.\d+(?:\.\d+)?', 7),  # VERSION
            (rb'\d+(?:\.\d+)?\s*(?:mg|g|kg|lbs?|oz|ml|l|gal|ft|feet|inches?|meters?|m|cm|mm)', 8),  # MEASUREMENT
            (rb'\d+\s*-\s*\d+', 9),  # RANGE
            (rb'\d{1,2}\s*CFR\s*\d{3,4}(?:\.\d+)?', 10),  # REGULATION
        ]
...
class FLPCExtractor:
    ...
    def __init__(self):
        ...
        self.patterns = {
            ...
            'RANGE': flpc.compile(r'\b\d+\s*-\s*\d+\b|\b\d+\s*to\s*\d+\b', flpc.IGNORECASE),
            'REGULATION': flpc.compile(r'\b\d{1,2}\s*CFR\s*\d{3,4}(?:\.\d+)?(?:\([a-z]\))?', flpc.IGNORECASE),
        }
